...
/*   Updated: 2025/01/13 19:36:37 by ogrativ          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../include/minishell.h"

t_env	*init_ls_colors(void)
{
	t_env	*env;
	...
	if (env == NULL)
		return (NULL);
	env->key = ft_strdup("LS_COLORS");
	if (env->key[0] == '\0')
		return (free_env(env), NULL);
	env->value = ft_strdup("rs=0:di=01;34:ln=01;36:mh=00:pi=40;33:so=01;35\
	...
	:*.opus=00;36:*.spx=00;36:*.xspf=00;36:*.c=01;35:*Makefile=38;5;214");
	if (env->value[0] == '\0')
		return (free_env(env), NULL);
	return (env);
}
...
int	init_env(t_list **lst, char *env[])
{
	...
	var = ft_get_env(*lst, "LS_COLORS");
	if (var != NULL)
		ft_env_unset(lst, "LS_COLORS");
	ft_lstadd_back(lst, ft_lstnew(init_ls_colors()));
	return (0);
}
